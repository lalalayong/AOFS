...
def gaussian_label_cpu(label, num_class, u=0, sig=4.0):
    """
    gaussian function encoding
    Args:
        label (float32): theta class
        num_theta_class (int): theta class num
        u (float32): center coordinate μ in gaussian function
        sig (float32): variance σ in gaussian function
    Returns:
        csl_label (array): [num_theta_class], gaussian function encoding
    """
...
def poly2rbox(polys, num_cls_thata=180, radius=6.0, use_pi=False, use_gaussian=False):
    ...
    Args:
        ...
        radius (float32): [1], radius (variance σ) in gaussian function
