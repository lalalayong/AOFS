...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            if cfg.tuning and isinstance(path, list):
                f = path
            else:
                f = []  # image files
            ...
            if cfg.tuning and isinstance(path, list):
                self.img_files = list(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in IMG_FORMATS)
            else:
        ...
        if cfg.tuning and isinstance(path, list):
            cache_path = Path(self.label_files[0]).parent.parent.joinpath('labelTxt_tuning').with_suffix('.cache')
        else:
        ...
        if cfg.tuning and isinstance(path, list):
            labels, shapes, self.segments = labels*cfg.repeat, shapes*cfg.repeat, self.segments*cfg.repeat
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # x_min=min(img_labels[0][1],img_labels[0][3],img_labels[0][5],img_labels[0][7])
            # y_min = min(img_labels[0][2], img_labels[0][4], img_labels[0][6], img_labels[0][8])
            # x_max = max(img_labels[0][1], img_labels[0][3], img_labels[0][5], img_labels[0][7])
            # y_max = max(img_labels[0][2], img_labels[0][4], img_labels[0][6], img_labels[0][8])
            # x_min=min(img_labels[1][1],img_labels[1][3],img_labels[1][5],img_labels[1][7])
            # y_min = min(img_labels[1][2], img_labels[1][4], img_labels[1][6], img_labels[1][8])
            # x_max = max(img_labels[1][1], img_labels[1][3], img_labels[1][5], img_labels[1][7])
            # y_max = max(img_labels[1][2], img_labels[1][4], img_labels[1][6], img_labels[1][8])
            ...
            img, (h0, w0), (h, w), img_labels = load_image_label(self, index)
            # x_min=min(img_labels[1][1],img_labels[1][3],img_labels[1][5],img_labels[1][7])
            # y_min = min(img_labels[1][2], img_labels[1][4], img_labels[1][6], img_labels[1][8])
            # x_max = max(img_labels[1][1], img_labels[1][3], img_labels[1][5], img_labels[1][7])
            # y_max = max(img_labels[1][2], img_labels[1][4], img_labels[1][6], img_labels[1][8])
            ...
            # cv2.imshow('123',img)
            # cv2.waitKey()
            img_label = np.empty([0, 9], dtype=float)
            for img_lab in img_labels:
                clsid = int(img_lab[0])
                if clsid not in cfg.base_ids:
                    continue
                img_label = np.append(img_label, [img_lab], axis=0)
        ...
        # labels_out = labels_out.repeat(len(cfg.base_ids), 1)
    ...
    def collate_fn(batch):
        ...
        # for i, l in enumerate(label):
        #     start = 0
        #     num = len(l)//len(cfg.base_ids)
        #     for k in range(len(cfg.base_ids)):
        #         l[start:start+num, 0] = j
        #         start = start + num
        #         j = j + 1
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w), img_labels = load_image_label(self, index)
        img_label = np.empty([0, 9], dtype=float)
        for img_lab in img_labels:
            clsid = int(img_lab[0])
            if clsid not in cfg.base_ids:
...
def verify_image_label(args):
    ...
    try:
        ...
        if im.format.lower() in ('jpg', 'jpeg'):
            with open(im_file, 'rb') as f1:
                f1.seek(-2, 2)
                if f1.read() != b'\xff\xd9':  # corrupt JPEG
        ...
        if os.path.isfile(lb_file):
            nf = 1  # label found
            with open(lb_file) as f2:
                labels = [x.split() for x in f2.read().strip().splitlines() if len(x)]
