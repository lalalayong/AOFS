...
def run(cfgdata=None,
    ...
    metaset = MetaDataset(metafiles=metadict, imgsz=imgsz[0], train=False, ensemble=True, with_ids=True)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        pred = model._forward_once(im, dynamic_weights, visualize=visualize)
        t3 = time_sync()
        dt[1] += t3 - t2
            ...
            if len(det):
                ...
                for *poly, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        poly = [x.tolist() for x in poly]
                        # line = (cls, *poly, conf) if save_conf else (cls, *poly)  # label format
                        # line = (poly[0], poly[1], poly[2], poly[3], poly[4], poly[5], poly[6], poly[7], names[int(cls)], 0)
                        x1, y1, x2, y2, x3, y3, x4, y4 = poly[0], poly[1], poly[2], poly[3], poly[4], poly[5], poly[6], poly[7]
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(f'{x1} {y1} {x2} {y2} {x3} {y3} {x4} {y4} {names[int(cls)]} 0\n')
...
def parse_opt():
    ...
    parser.add_argument('--cfgdata', type=str, default=ROOT / 'cfg/fewyolov5_nwpu.data', help='datacfg .data path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/weights/best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'dataset/dataset_demo/images', help='file/dir/URL/glob, 0 for webcam')
