...
def train(hyp, opt, device, callbacks):
    datacfg, save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg1, cfg2, resume, noval, nosave, workers, freeze = \
        Path(opt.cfgdata), opt.save_dir, opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg1, opt.cfg2, \
    ...
    if pretrained:
        ...
        # csd_1 = ckpt['model_reweight'].float().state_dict()
        # csd.update(**csd_1)
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect
        model.load_state_dict(csd, strict=False)  # load
        LOGGER.info(f'Transferred {len(csd)}/{len(model.state_dict())} items from {weights}')  # report
    else:
        model = Model(cfg1, cfg2, ch1=3, ch2=4, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    ...
    # Metaloader
    metaset = MetaDataset(metafiles=metadict, imgsz=imgsz, train=True)
    ...
    LOGGER.info(f'Image sizes {imgsz} train, {imgsz} val\n'
                f'Using {train_loader.num_workers * WORLD_SIZE} train_loader workers\n'
                f'Using {num_workers} meta_loader workers\n'
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg1', type=str, default='', help='model.yaml path')
    parser.add_argument('--cfg2', type=str, default='', help='reweight_model.yaml path')
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_wandb_resume(opt) and not opt.evolve:  # resume an interrupted run
        ...
        opt.data, opt.cfg1, opt.cfg2, opt.hyp, opt.weights, opt.project = \
            check_file(opt.data), check_yaml(opt.cfg1), check_yaml(opt.cfg2), check_yaml(opt.hyp), str(opt.weights), str(opt.project)  # checks
        assert len(opt.cfg1) or len(opt.weights), 'either --cfg or --weights must be specified'
        if opt.evolve:
