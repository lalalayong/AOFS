...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, stride, names, single_cls, pad=pad, rect=False,
                                       workers=workers, prefix=colorstr(f'{task}: '))[0]

        metaset = MetaDataset(metafiles=metadict, imgsz=imgsz, train=False, ensemble=True, with_ids=True)
    ...
    # Generate dynamic_weights
    n_cls = len(metaset.classes)
    # n_cls = len(cfg.classes)

    enews = []
    for i in range(3):
        enews.append([0.0] * n_cls)
    cnt = [0.0] * n_cls
    # print('===> Generating dynamic weights...')
    pbar = enumerate(metaloader)
    nb = len(metaset) // (metaset.batch_size)
    pbar = tqdm(pbar, total     =nb, desc='===> Generating dynamic weights')
    # kkk = 0
    for i,(metax, mask, clsids) in pbar:
        # print('===> {}/{}'.format(kkk, len(metaset) // metaset.batch_size))
        # kkk += 1
        # metax, mask = metax.cuda(), mask.cuda()
        metax = metax.to(device, non_blocking=True)
        mask = mask.to(device, non_blocking=True)
        metax = metax.half() if half else metax.float()
        mask = mask.half() if half else mask.float()
        metax /= 255
        # metax, mask = Variable(metax, volatile=True), Variable(mask, volatile=True)
        dws = model.meta_forward(metax, mask)
        for ci, c in enumerate(clsids):
            for i in range(3):
                enews[i][c] = enews[i][c] * cnt[c] / (cnt[c] + 1) + dws[i][ci] / (cnt[c] + 1)
            cnt[c] += 1
    dynamic_weights = []
    for i in range(3):
        dynamic_weights.append(torch.stack(enews[i]))
    ...
    s = ('%25s' + '%11s' * 6) % ('Class', 'Images', 'Labels', 'P', 'R', '  HBBmAP@.5', '  HBBmAP@.5:.95')
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        out, train_out = model._forward_once(im, dynamic_weights) if training else model._forward_once(im, dynamic_weights, val=True)
        dt[1] += time_sync() - t2
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [xylsθ, conf, cls])
            ...
            # path, shape = Path(paths[(si // len(cfg.base_ids))]), shapes[(si // len(cfg.base_ids))][0] # shape (tensor): (h_raw, w_raw)
            seen += 1
            ...
            # scale_polys(im[(si // len(cfg.base_ids))].shape[1:], pred_polyn[:, :8], shape, shapes[(si // len(cfg.base_ids))][1])  # native-space pred
            ...
            if nl:
                ...
                # scale_coords(im[(si // len(cfg.base_ids))].shape[1:], tbox, shape, shapes[(si // len(cfg.base_ids))][1])  # native-space labels
            ...
            # callbacks.run('on_val_image_end', pred_hbb, pred_hbbn, path, names, im[(si // len(cfg.base_ids))])
